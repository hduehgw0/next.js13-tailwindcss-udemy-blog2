    ...
    <div>
        <article className='shadow my-4' key={article.id}>
            <Link href={`articles/${article.id}`} className='hover:opacity-75'>
                <Image 
                    src={`https://picsum.photos/1000/500?random=${article.id}`}
                    alt=""
                    width={1280}
                    height={300}
                />
            ...
            <div className='bg-white flex flex-col justify-start p-6'>
                ...
                <Link href={`articles/${article.id}`} className='text-gray-900 pb-6'>
                    {article.content.length > 70 ? article.content.substring(0,70) + "..." : article.content}
                </Link>
                <Link href={`articles/${article.id}`} className='text-orange-800 hover:text-black'>続きを読む</Link>
            </div>
        </article>
    </div>
)}
